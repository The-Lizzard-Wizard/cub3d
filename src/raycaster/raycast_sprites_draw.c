...
/*   Updated: 2026/01/09 16:41:21 by authomas         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
			...
			t_raycast *raycast, t_thing *thing_curr)
{
	...
	{
		sp_cast->tex_x = (int)(sp_cast->tex_step_x * (sp_cast->stripe
					- (sp_cast->sp_sc_x - sp_cast->sp_w / 2)));
		if (sp_cast->transform.y > 0
			&& sp_cast->stripe > 0 && sp_cast->stripe <= WIN_W
			&& sp_cast->transform.y < raycast->z_buffer[sp_cast->stripe])
		{
			sp_cast->tex_pos = (sp_cast->sp_start_y - WIN_H / 2
					+ sp_cast->sp_h / 2) * sp_cast->tex_step_y;
